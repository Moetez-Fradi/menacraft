...
const SOURCE_CREDIBILITY_URL = process.env.SOURCE_CREDIBILITY_URL ?? "http://localhost:8084";
...
export async function POST(req: NextRequest) {
  ...
  const downstream = await callDownstream(`${SOURCE_CREDIBILITY_URL}/analyze`, payload);

  if (!downstream.ok) {
    return NextResponse.json(
      {
        error: downstream.error ?? "source_credibility_failed",
        service: downstream,
        explainability: {
